...
!-----------------------------------------------------------------------
      parameter (nmo=num_t,lx=num_x,ly=num_y,mx=num_x,my=3,mode=10,mnu=1)
      parameter (jn=lx*3,nm=(jn*(jn+1))/2,nm2=nm*2)
      dimension xin2(mx,my),xin(jn),xin3(jn),udata(mx,my,nmo)
      dimension ain2(mx,my),ain(jn),ain3(jn),adata(mx,my,nmo)
      dimension xiin(lx,ly),xjjn(mx,my), aiin(lx,ly), ajjn(mx,my)
      dimension olrin(lx,ly), u850in(lx,ly), u200in(lx,ly)
      dimension olran(lx,ly), u850an(lx,ly), u200an(lx,ly)
      dimension d(jn),wk(nm2),z(jn,jn),tmp(lx,ly)
      dimension ev(jn),t(mode),vt(jn,jn),xin1(jn), at(mode), ain1(jn)
      dimension var_olr(mode), var_u850(mode), var_u200(mode)
      real ave_xjjn(mx,my), n_ave_xjjn(mx,my)
      real std_xjjn(mx,my), n_std_xjjn(mx,my)
      real zm_std(my)
      real total_var_olr, total_var_u850, total_var_u200, tmp_var
      common /a1/sxy(nm),sm(jn),zc(jn),zz(jn),dum(mx,my)
      common /a2/dmis(mx,my),ddo(jn),ddo1(nm)
      real lat,pi
      character*150 olrinput, u850input, u200input, &
           olrano, u850ano, u200ano
      character*3 mtrix
      ...
      character*80 outd
      data olrinput &
           /'homedir/level_1/olr_name/data/&
           daily.filt.20-100.lanz.100.15S15N.period.gdat'/ 
      data u850input &
           /'homedir/level_1/u850_name/data/&
           daily.filt.20-100.lanz.100.15S15N.period.gdat'/ 
      data u200input &
           /'homedir/level_1/u200_name/data/&
           daily.filt.20-100.lanz.100.15S15N.period.gdat'/ 

      data olrano &
           /'homedir/level_1/olr_name/data/&
           daily.anom.15S15N.period.gdat'/ 
      data u850ano &
           & /'homedir/level_1/u850_name/data/&
           daily.anom.15S15N.period.gdat'/ 
      data u200ano &
...
! KIM [07/05/25] square-root of the zonal mean of 
! their temporal variance 
      open (10, file=trim(outd)//'.var', &
              status='unknown')

! KIM [07/01/01]  variance explained 
! by each of the input fields
      open(7,file=trim(outd)//'.pct.olr', &
              status='unknown')
      open(8,file=trim(outd)//'.pct.u850', &
              status='unknown')
      open(9,file=trim(outd)//'.pct.u200', &
              status='unknown')
...
! ************* input data reading.... *********************
      open(11,file=olrinput,form='unformatted',status='old', &
            access='direct',recl=lx*ly*linux)
      open(12,file=u850input,form='unformatted',status='old', &
            access='direct',recl=lx*ly*linux)
      open(13,file=u200input,form='unformatted',status='old', &
            access='direct',recl=lx*ly*linux)
      open(14,file=olrano,form='unformatted',status='old', &
            access='direct',recl=lx*ly*linux)
      open(15,file=u850ano,form='unformatted',status='old', &
            access='direct',recl=lx*ly*linux)
      open(16,file=u200ano,form='unformatted',status='old', &
            access='direct',recl=lx*ly*linux)
      ...
      do it = 1, nmo
       ...
       ii = it
         ...
         read(11,rec=ii) olrin
         read(12,rec=ii) u850in
         read(13,rec=ii) u200in
         read(14,rec=ii) olran
         read(15,rec=ii) u850an
         read(16,rec=ii) u200an

         iy=0
         do j=1,my
            lat=0. !  latitude weighting
            iy=iy+1
         ...
         do i=1,mx
            ix=ix+1
          if (j.eq.1) then
            xjjn(ix,iy)=olrin(i,1)
          elseif (j.eq.2) then
            xjjn(ix,iy)=u850in(i,1)
          elseif (j.eq.3) then
            xjjn(ix,iy)=u200in(i,1)
          endif

          if (j.eq.1) then
            ajjn(ix,iy)=olran(i,1)  
          elseif (j.eq.2) then
            ajjn(ix,iy)=u850an(i,1)  
          elseif (j.eq.3) then
            ajjn(ix,iy)=u200an(i,1)  
          endif
         ...
         do j = 1, my
         do i = 1, mx
          if (xjjn(i,j).ne.dmiss) then
           ave_xjjn(i,j) = ave_xjjn(i,j) + xjjn(i,j)
           n_ave_xjjn(i,j) = n_ave_xjjn(i,j) + 1
          endif
         enddo
         enddo
...
! averging
       do j = 1, my
       do i = 1, mx
        if (n_ave_xjjn(i,j).ne.0) then
         ave_xjjn(i,j) = ave_xjjn(i,j)/n_ave_xjjn(i,j)
        else
         ave_xjjn(i,j) = dmiss
        endif
       enddo ! i
      ...
      do it = 1, nmo
       ...
       ii = it
         ...
         do j = 1, my
         do i = 1, mx
          if (xjjn(i,j).ne.dmiss.or.ave_xjjn(i,j).ne.dmiss) then
           std_xjjn(i,j) = std_xjjn(i,j) + &
      (xjjn(i,j)-ave_xjjn(i,j))*(xjjn(i,j)-ave_xjjn(i,j))
           n_std_xjjn(i,j) = n_std_xjjn(i,j) + 1
          else
           print*,'missing?'
          endif
         enddo
...
! std
       do j = 1, my
       do i = 1, mx
        if (n_std_xjjn(i,j).ne.0) then
         std_xjjn(i,j) = std_xjjn(i,j)/n_std_xjjn(i,j)
        else
         std_xjjn(i,j) = dmiss
        endif
       enddo ! i
       ...
       do j = 1, my
       do i = 1, mx
        zm_std(j) = zm_std(j) + std_xjjn(i,j)/float(mx)
       enddo ! i
       enddo ! j

       do j = 1, my
        zm_std(j) = sqrt(zm_std(j))
       enddo

! square-root of the zonal mean of their temporal variance 
       write (10, 17) (zm_std(j),j=1,my)
       write (10, 17) (zm_std(j),j=1,my)
       write (10, 17) (zm_std(j),j=1,my)
      ...
      do it = 1, nmo
       ...
       ii = it
         ...
         do i=1,mx
            ix=ix+1
          if (j.eq.1) then
            xjjn(ix,iy)=olrin(i,1)/zm_std(j)
          elseif (j.eq.2) then
            xjjn(ix,iy)=u850in(i,1)/zm_std(j)
          elseif (j.eq.3) then
            xjjn(ix,iy)=u200in(i,1)/zm_std(j)
          endif
         ...
         do j = 1, my
         do i = 1, mx
          if (xjjn(i,j).ne.dmiss.and.ave_xjjn(i,j).ne.dmiss) then
           std_xjjn(i,j) = std_xjjn(i,j) + &
      (xjjn(i,j)-ave_xjjn(i,j))*(xjjn(i,j)-ave_xjjn(i,j))
           n_std_xjjn(i,j) = n_std_xjjn(i,j) + 1
          endif
...
112   continue
      ...
      do k = 1, mx
       total_var_olr = total_var_olr + zc(k)*zc(k)/float(mx*nmo)
       total_var_u850 = total_var_u850 + &
      zc(mx+k)*zc(mx+k)/float(mx*nmo)
       total_var_u200 = total_var_u200 + &
      zc(2*mx+k)*zc(2*mx+k)/float(mx*nmo)
      enddo
...
! pc when projected to raw data (not filtered)
 ...
 129  continue
      ...
      do i = 1, mx
      tmp_var = dum(i,1)*sqrt(d(nl-k+1))
      var_olr(k) =  var_olr(k) + tmp_var*tmp_var/float(mx)
      tmp_var = dum(i,2)*sqrt(d(nl-k+1))
      var_u850(k) =  var_u850(k) + tmp_var*tmp_var/float(mx)
      tmp_var = dum(i,3)*sqrt(d(nl-k+1))
      var_u200(k) =  var_u200(k) + tmp_var*tmp_var/float(mx)
      enddo
      ...
      print*,'olr',k,ev(k)
      enddo
      write(7,17) (ev(i),i=1,mode)  !eigen value (pct)                         
      write(7,17) (ev(i),i=1,mode)  !eigen value (pct)                         
      write(7,17) (ev(i),i=1,mode)  !eigen value (pct)                         
      do k = 1, 10
      ev(k) = var_u850(k)*100
      print*,'u850',k,ev(k)
      enddo
      write(8,17) (ev(i),i=1,mode)  !eigen value (pct)                         
      write(8,17) (ev(i),i=1,mode)  !eigen value (pct)                         
      write(8,17) (ev(i),i=1,mode)  !eigen value (pct)                         
      do k = 1, 10
      ev(k) = var_u200(k)*100
      print*,'u200',k,ev(k)
      enddo
      write(9,17) (ev(i),i=1,mode)  !eigen value (pct)                         
      write(9,17) (ev(i),i=1,mode)  !eigen value (pct)                         
      write(9,17) (ev(i),i=1,mode)  !eigen value (pct)                         
