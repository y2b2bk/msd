...
!     real OEE(NT+1,NL/2+1)
      ...
      integer year1, leap, linux, season, dmax
      parameter (year1 = beg_year, leap = leap_year)
      parameter (linux = linux_recl, season = sea_num)
      parameter (dmax = num_d)
      integer yy, dd, nod, it, skip
...
! taper first and last 45 days with cosine window
      do n=1,NL
      do t=1,NT
       if (t.ge.1.and.t.le.45+1) then
       EEr(n,t) = EEr(n,t)*0.5*(1.-cos((t-1)*Pi/float(45)))
       elseif (t.ge.nt-45.and.t.le.nt) then
       EEr(n,t) = EEr(n,t)*0.5*(1.-cos((nt-t)*Pi/float(45)))
       endif
      enddo
      enddo
...
!--------------------------------------------------------------------
      ...
      write  (12,1234) FOUT,21,(ff(i),i=nt/2+1-10,nt/2+1+10) &
