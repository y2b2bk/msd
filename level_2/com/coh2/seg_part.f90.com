	...
	real undef
        ...
        open (1, &
        file='homedir/level_1/variable/data/daily.anom.period.gdat',&
        form='unformatted',access='direct',recl=nx*ny*linux_recl,&
        status='old')

        open (11,&
        file='homedir/level_2/coh2/variable/data/seg256_over206_sym_asy.gdat',&
...
! write_seg
	...
	do it=1,nt
	 irec=irec+1
         ...
         do j = 1, jj
         do i = 1, nx
           if (j.ge.1.and.j.le.(jj/2+1)) then
            com(i,j,it) = sym(i,j,it)
           else
            com(i,j,it) = asy(i,j,it)
           endif
         enddo
         enddo

	 write (11,rec=irec) ((com(i,j,it),i=1,ii),j=1,jj)
	enddo
...
20     continue
	print*,iseg,'= total 256-day segments'
