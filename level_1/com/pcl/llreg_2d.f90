...
! laged linear regression
 ...
 integer :: i, j, iz, it, inv, ilag, iy, id, dd, nd, yy
...
! smax : number of total record in output file (=ksel*vmax)
 ...
 namelist /dimension_nml/ imax, jmax, kmax, ksel, vmax, rmax, smax, &
  dmax, tmax, filt_wing, ymax, season, year1, leap, linux, fac1, fac2
...
n_var2 = 0.
 ...
 if (id+ilag.gt.0.and.id+ilag.lt.nd+1) then  ! if-ilag
...
n_bb = 0.
 ...
 if (id+ilag.gt.0.and.id+ilag.lt.nd+1) then
...
! for leap year winter season
    if (leap.eq.1.and.season.eq.1) then
     dd = dd + nd
    endif
